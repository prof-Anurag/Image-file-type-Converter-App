...
with drag-and-drop functionality and modern UI design.
"""
...
class ImageConverterApp:
    ...
    def create_ui(self):
        ...
        subtitle_label = ctk.CTkLabel(
            title_frame,
            text="Drag & Drop images or click to browse • Convert to multiple formats",
    ...
    def create_drag_drop_area(self, parent):
        ...
        # Create animated drop zone with better visual feedback
        ...
        # Drag and drop label with better styling
        ...
        # Make the drop area clickable with hover effect
        drop_frame.bind("<Button-1>", self.browse_files)
        self.drop_label.bind("<Button-1>", self.browse_files)
        drop_frame.bind("<Enter>", self.on_drop_hover)
        drop_frame.bind("<Leave>", self.on_drop_leave)
        
        # Store reference for drag-drop highlighting
    ...
    def on_drop(self, event):
        ...
        self.root.after(1000, self.reset_drop_area)
        
    def on_drop_hover(self, event):
        """Handle mouse hover over drop area"""
        self.drop_frame.configure(
            border_color=["#4A9EFF", "#2E7AD1"]
        )
        
    def on_drop_leave(self, event):
        """Handle mouse leave drop area"""
        self.drop_frame.configure(
            border_color=["gray70", "gray30"]
        )
        
    def reset_drop_area(self):
        """Reset drop area to normal state"""
        self.drop_frame.configure(
            fg_color=["gray92", "gray14"],
            border_color=["gray70", "gray30"]
        )
